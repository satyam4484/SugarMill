...
export const createCustomer = async (customerData: ICustomerRequest): Promise<any> => {
    ...
    try {
        ...
        // check if email or contactNo already exists in the database
        const existingEmail = await CustomerRequest.findOne({ email: customerData.email });
        if (existingEmail) {
        ...
        const existingContactNo = await CustomerRequest.findOne({ contactNo: customerData.contactNo });
